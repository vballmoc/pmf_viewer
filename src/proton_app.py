...
# - Leak uses a soft threshold:
#       nearly no leak below threshold,
#       progressively stronger leak above threshold.
...
def soft_threshold_leak(psi_mV, conductance, threshold_mV, softness_mV):
    ...
    The leak always dissipates the membrane potential.

    For positive-inside Δψ:
        leak is negative, i.e. H+ leaves the vesicle.

    For negative-inside Δψ:
        leak is positive, i.e. H+ enters the vesicle.

    The softplus function gives a smooth approximation of:
        max(0, |Δψ| - threshold)
    """
    abs_psi = np.abs(psi_mV)

    # Numerically stable softplus:
    # softness * log(1 + exp((x-threshold)/softness))
...
# Prevent division by zero in extremely unbuffered edge cases
if beta_total <= 0:
    beta_total = 1e-30
...
for i in range(1, n_points):
    ...
    # Soft-threshold leak: always dissipates Δψ.
    leak_flux_i = soft_threshold_leak(
        psi,
        leak_conductance,
        leak_threshold_mV,
        leak_softness_mV
