...
outer_diameter_nm = st.sidebar.slider("Outer vesicle diameter / nm", 25, 1000, 100, step=25)
bilayer_thickness_nm = st.sidebar.slider("Bilayer thickness / nm", 2.0, 8.0, 4.5, step=0.1)
...
# The outside diameter includes the bilayer. The aqueous lumen is smaller
# by one bilayer thickness on each side.
inner_diameter_nm = max(outer_diameter_nm - 2 * bilayer_thickness_nm, 0.1)
...
st.info(
    f"For a {outer_diameter_nm:.0f} nm outer-diameter vesicle with {inner_diameter_nm:.1f} nm aqueous lumen at {K_in_initial_mM:.1f} mM K⁺, "
...
For this vesicle aqueous lumen, the initial internal K⁺ pool is approximately
